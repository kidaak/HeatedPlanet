...
public class GridDisplay extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		if (grid != null) {
			...
			double degFromLeft = grid.getSunPositionDeg()+180f;
			int sunx = (int) Math.round(degFromLeft * pixPerDeg);
			g.drawLine(sunx, 0, sunx, h);

			// Draw sun position latitude
			g.setColor(Color.YELLOW);
			pixPerDeg = h/180.0f;
			double degFromMiddle = grid.getSunLatitudeDeg();//+180f;
			int suny = (int) Math.round(-degFromMiddle * pixPerDeg + h/2);
